...
blowFish.ROUNDS = 16;
blowFish.BLOCK_SIZE = 8; // bytes = 64 bits
blowFish.SBOX_SK = 256;
blowFish.P_SZ = blowFish.ROUNDS + 2;
blowFish.S0 = null;
blowFish.S1 = null;
blowFish.S2 = null;
blowFish.S3 = null;
blowFish.P = null; // the p-array
blowFish.workingKey = null;

blowFish.processTable = function (xl, xr, table) {
    var size = table.length;
    for (var s = 0; s < size; s += 2) {
        xl ^= blowFish.P[0];
        xr ^= blowFish.func(xl) ^ blowFish.P[1];
        xl ^= blowFish.func(xr) ^ blowFish.P[2];
        xr ^= blowFish.func(xl) ^ blowFish.P[3];
        xl ^= blowFish.func(xr) ^ blowFish.P[4];
        xr ^= blowFish.func(xl) ^ blowFish.P[5];
        xl ^= blowFish.func(xr) ^ blowFish.P[6];
        xr ^= blowFish.func(xl) ^ blowFish.P[7];
        xl ^= blowFish.func(xr) ^ blowFish.P[8];
        xr ^= blowFish.func(xl) ^ blowFish.P[9];
        xl ^= blowFish.func(xr) ^ blowFish.P[10];
        xr ^= blowFish.func(xl) ^ blowFish.P[11];
        xl ^= blowFish.func(xr) ^ blowFish.P[12];
        xr ^= blowFish.func(xl) ^ blowFish.P[13];
        xl ^= blowFish.func(xr) ^ blowFish.P[14];
        xr ^= blowFish.func(xl) ^ blowFish.P[15];
        xl ^= blowFish.func(xr) ^ blowFish.P[16];
        xr ^= blowFish.P[17];
...
blowFish.setKey = function (key) {
     ...
     */
    blowFish.S0 = blowFish.KS0.slice();
    blowFish.S1 = blowFish.KS1.slice();
    blowFish.S2 = blowFish.KS2.slice();
    blowFish.S3 = blowFish.KS3.slice();
    blowFish.P = blowFish.KP.slice();
    /**
     ...
     */
    blowFish.processTable(0, 0, blowFish.P);
    blowFish.processTable(blowFish.P[blowFish.P_SZ - 2], blowFish.P[blowFish.P_SZ - 1], blowFish.S0);
    blowFish.processTable(blowFish.S0[blowFish.SBOX_SK - 2], blowFish.S0[blowFish.SBOX_SK - 1], blowFish.S1);
    blowFish.processTable(blowFish.S1[blowFish.SBOX_SK - 2], blowFish.S1[blowFish.SBOX_SK - 1], blowFish.S2);
    blowFish.processTable(blowFish.S2[blowFish.SBOX_SK - 2], blowFish.S2[blowFish.SBOX_SK - 1], blowFish.S3);
}

blowFish.init = function (key) {

    blowFish.S0 = new Array(blowFish.SBOX_SK);
    blowFish.S1 = new Array(blowFish.SBOX_SK);
    blowFish.S2 = new Array(blowFish.SBOX_SK);
    blowFish.S3 = new Array(blowFish.SBOX_SK);
    blowFish.P = new Array(blowFish.P_SZ);

    blowFish.workingKey = key;
    blowFish.setKey(blowFish.workingKey);
}

blowFish.getAlgorithmName = function () {
    return "Blowfish";
}

blowFish.getBlockSize = function () {
    return blowFish.BLOCK_SIZE;
}

blowFish.func = function (x) {
    return (((blowFish.S0[(x >>> 24)] + blowFish.S1[(x >>> 16) & 0xff]) ^ blowFish.S2[(x >>> 8) & 0xff]) + blowFish.S3[x & 0xff]);
}

blowFish.encrypt = function (raw, offset, size) {
    for (var i = offset; i < (offset + size); i += 8) {
        blowFish.encryptBlock(raw, i);
    }
}

blowFish.encryptBlock = function (src, srcIndex, dst, dstIndex) {
    if (!dst) dst = src;
    if (!dstIndex) dstIndex = srcIndex;

    var xl = blowFish.bytesTo32bits(src, srcIndex);
    var xr = blowFish.bytesTo32bits(src, srcIndex + 4);
    ...
    blowFish.bits32ToBytes(xr, dst, dstIndex);
    blowFish.bits32ToBytes(xl, dst, dstIndex + 4);
}

blowFish.decryptBlock = function (src, srcIndex, dst, dstIndex) {
    if (!dst) dst = src;
    if (!dstIndex) dstIndex = srcIndex;
    ...
    xl ^= blowFish.P[17];
    xr ^= blowFish.func(xl) ^ blowFish.P[16];
    xl ^= blowFish.func(xr) ^ blowFish.P[15];
    xr ^= blowFish.func(xl) ^ blowFish.P[14];
    xl ^= blowFish.func(xr) ^ blowFish.P[13];
    xr ^= blowFish.func(xl) ^ blowFish.P[12];
    xl ^= blowFish.func(xr) ^ blowFish.P[11];
    xr ^= blowFish.func(xl) ^ blowFish.P[10];
    xl ^= blowFish.func(xr) ^ blowFish.P[9];
    xr ^= blowFish.func(xl) ^ blowFish.P[8];
    xl ^= blowFish.func(xr) ^ blowFish.P[7];
    xr ^= blowFish.func(xl) ^ blowFish.P[6];
    xl ^= blowFish.func(xr) ^ blowFish.P[5];
    xr ^= blowFish.func(xl) ^ blowFish.P[4];
    xl ^= blowFish.func(xr) ^ blowFish.P[3];
    xr ^= blowFish.func(xl) ^ blowFish.P[2];
    xl ^= blowFish.func(xr) ^ blowFish.P[1];
    xr ^= blowFish.P[0];
...
blowFish.decrypt = function (raw, offset, size) {
    for (var i = offset; i < (offset + size); i += 8) {
        blowFish.decryptBlock(raw, i);
    }
}

blowFish.bits32ToBytes = function (ind, dst, dstIndex) {
    dst[dstIndex] = ind;
    dst[dstIndex + 1] = (ind >> 8);
    dst[dstIndex + 2] = (ind >> 16);
    dst[dstIndex + 3] = (ind >> 24);
}

blowFish.bytesTo32bits = function (src, srcIndex) {
